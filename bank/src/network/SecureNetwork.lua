...
local function serializeEncryptionKeys(dataCard, encryptionKeys)
    return serialization.serialize({public = encryptionKeys.public.serialize(),
...
function SecureNetwork:init(encryptionKeyFile, network, dataCard)
    local obj = {}
...
function SecureNetwork:receive(timeout)
    return self._network:receive(timeout)
end
...
function SecureNetwork:connect(address)
    ...
    self._network:send(address, serialization.serialize({
        type = "key_exchange",
        publicKey = sPublickey.serialize()
    }))
    ...
    while i < 5 do
        ...
        if returnAddress == address then
            ...
            if type(msg) == "table" and msg.type == "key_exchange_response" then
                obj.remotePublicKey = self.dataCard.deserializeKey(msg.publicKey,
                                                                   "ec-public")
...
function SecureNetwork:handleIncoming(senderAddress, message)
    local msg = serialization.unserialize(message)
    ...
    if msg.type == "key_exchange" then
        ...
        conn.remotePublicKey = self.dataCard.deserializeKey(msg.publicKey, "ec-public")
        os.sleep(1)
        self:send(senderAddress, serialization.serialize({
            type = "key_exchange_response",
            publicKey = conn.sessionKey.public.serialize()
        }))
