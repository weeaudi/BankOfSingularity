...
local function fetch(toAddr, port, packet)
    ...
    if serverAddr ~= toAddr or serverPort ~= port then goto pullevent end
...
local function discoverServer()
    ...
    if not serverAddr then return nil end

    if msg ~= 'BANK_HERE' then return nil end
...
while not server do server = discoverServer() end

local req = Protocol.makeRequest('GetCardsByAccountId', modem.address, server,
                                 {accountId = 42}, nil)
