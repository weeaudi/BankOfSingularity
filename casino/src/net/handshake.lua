...
-- ^ computer.uptime() is an OC API extension; dataComp unused in announce() by design
--- casino/src/net/handshake.lua
--- Device authentication and AES session key establishment.
---
--- Two-phase protocol:
---
---   Phase 1 — Announce (port 102):
---     Device sends {type="device_announce", deviceId, deviceType, publicKey}.
---     Server replies {type="announce_ack", status}.
---     Status is one of: "pending" | "active" | "revoked" | "suspended".
---     Repeat until "active" (admin must approve new devices).
---
---   Phase 2 — Handshake (port 101):
---     Device generates an ephemeral EC key pair, signs it with its identity
---     private key (ECDSA + timestamp to prevent replay), and sends it.
---     Server responds with its own ephemeral public key.
---     Both sides run ECDH and derive the same 16-byte AES-128 key via SHA-256.
---
--- After the handshake all RPC traffic on port 100 is AES-128-CBC encrypted
--- with a fresh random IV per message ("ivB64:ciphertextB64" wire format).
